...
Marking your account private will make all lock commands send in private messages (e.g. `/lock` and `/createlock`) that only you can see. In addition users can not look at your user profile unless they are in your [trusted user list](/docs/user-settings/trusted-users) or are a pending or approved keyholder on a current lock.
